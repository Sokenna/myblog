...
- 6.3 npm与包
  - 6.3.1 包
    - 6.3.1.1 什么是包
      - Node.js的第三方模块又叫做包。
      - 第三方模块和包是同一个概念，只不过叫法不同
    - 6.3.1.2 包的来源
      - 不同于Node.js中的内置模块和自定义模块，包是由第三方个人或团队开发的，免费供所有人使用
      - ==注意：== Node.js中的包都是免费且开源的，不需要付费即可免费下载使用
    - 6.3.1.3 为什么需要包
      - 由于Node.js的内置模块仅提供了一些底层API，导致在基于内置模块进行项目开发时，效率很低。
      - 包是基于内置模块的API封装出来的，提供了更高级、更方便的API，极大的提高了开发效率
      - 包和模块之间的关系，类似于jQuery和浏览器内置API之间的关系
    - 6.3.1.4 从哪里下载包
      - https://www.npmjs.com 搜索包
      - https://registry.npmjs.org/ 下载包
    - 6.3.1.5 如何下载包
      - npm,inc公司提供了一个包管理工具，我们可以使用这个包管理工具，从https://registry.npmjs.org/服务器下载所需包到本地
      - 这个包管理工具叫Node Package Manager(简称npm包管理工具)，这个包管理工随着Node.js的安装包一起被安装到了用户的电脑上。
      - 在终端通过npm -v 可以查看本地安装的npm的版本
  - 6.3.2 npm初体验
    - 6.3.2.1
    - 6.3.2.6 包的语义化版本规范
      - 包的版本号是以 "点分十进制" 形式进行定义的，总共三位数，例如2.24.0
      - 其他每一位数字代表的含义如下：
        1. 第一位数字： 大版本
        2. 第二位数字：功能版本
        3. 第三位数字：Bug修复版本
    ...
    - 6.3.3 包管理配置文件
      - 6.3.3.1 npm规定，在项目跟目录中，必须提供一个package.json的包管理配置文件。用来记录与项目有关的一些配置信息。如：
        1. 项目的名称、版本号、描述等
        2. 项目中都用到了哪些包
        3. 哪些包只在开发期间用到
        4. 哪些包在开发和部署时都需要用到
      - 6.3.3.3 快速创建package.json
        - npm包管理工具提供了一个快捷命令，可以在执行命令时所处的目录中，快速创建package.json这个包管理配置文件：
        ```shell
            //作用：在执行命令所处的目录中，快速创建package.json配置文件
            npm init -y
        ```
        - ==注意== 
          1. 上述命令只能在英文的目录下成功运行。所以，项目文件夹的名称一定要使用英文命名，不用使用中文，不能出现空格。
          2. 运行npm install命令安装包的时候，npm包管理工具会自动把包的名称和版本号，记录到package.json中。
      - 6.3.7开发属于自己的包
        - 
        - 6.3.7.2 初始化包的基本结构
          - 新建snopy-tools文件夹，新建如下三个文件：
            1. package.json(包管理配置文件)
            2. index.js(包的入口文件)
            3. README.md(包的说明文档)
          - package.json 主要有以下几个重要信息
          ```json
            {
              "name": "snopy",
              "version": "1.0.0",
